...
void LeftLeaningRedBlack::TraverseRec(LLTB_t *pNode, uint32_t &prev)
{
	...
	prev = pNode->Ref.Key;
	printf("Value: %4d ", pNode->Ref.Key);
	//Outputs color and Parent and parent color
	if(pNode->IsRed == true)
	{
		cout << "Color: Red" << endl;
	}
	...
	if(findParent(pNode->Ref.Key) == nullptr)
	{
		cout << "This is a Root." << endl ;
	}
	else
	{
		cout << "The parent of " << pNode->Ref.Key << " is " << findParent(pNode->Ref.Key)->Ref.Key << " with " ;
	}
	
	if(findParent(pNode->Ref.Key) != nullptr)
	{
		if(findParent(pNode->Ref.Key)->IsRed == true)
		{
			cout << "Color: Red" << endl;
		}
		else
...
//Returns smaller of two unsigned 32 bit integers
uint32_t LeftLeaningRedBlack::Min(uint32_t a, uint32_t b)
{
	if(a < b)
	{
		return a;
...
//Returns larger of two unsigned 32 bit ints
uint32_t LeftLeaningRedBlack::Max(uint32_t a, uint32_t b)
{
	if(a > b)
	{
		return a;
...
//Copied from QzCommon.h given to us
int LeftLeaningRedBlack::ArraySize(uint32_t array[])
{
	return sizeof(array)/(sizeof(array)[0]);
}

//Copied from QzCommon.h given to us
void LeftLeaningRedBlack::SafeDelete(LLTB_t *pNode)
{
	if (NULL != (pNode))
	{ 
		delete (pNode);     
...
//Find Parent function - finds parent node of a value/key
//Similar to Lookup but returns parent node of the node with a specific value/key
LLTB_t *LeftLeaningRedBlack::findParent(uint32_t key)
{
	LLTB_t *pNode = m_pRoot;

	//Checks if the Root is the same as the key, if equal returns nullptr because there is no parent of the root
	if(key == pNode->Ref.Key)
	{
		return NULL;
	}
	...
	while(pNode != NULL)
	{
		//If value is found in children, return parent node
		if(key == pNode->pLeft->Ref.Key || key == pNode->pRight->Ref.Key)
		{
			return pNode;
		}
		//Goes to check left side if key is smaller than root key
		else if(key < pNode->Ref.Key)
		{
			pNode = pNode->pLeft;
		}
		//Goes to check right side if Key is larger than root key
		else
		{
			pNode = pNode->pRight;
		}
...
//Finds node of specific inserted value
//same as lookup but returns lltb_t
LLTB_t *LeftLeaningRedBlack::findInsert(uint32_t key)
{
	LLTB_t *pNode = m_pRoot;
